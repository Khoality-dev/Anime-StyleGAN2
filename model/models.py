...
class ModulatedConv2D(nn.Module):
    ...
    def forward(self, x, sy, sx, noise):
        ...
        #b = self.b.unsqueeze(0).reshape(1, -1, 1, 1)
        #x = x.add(b)
        #x = x.add(noise)
        #x = self.activation(x)
        ...
        x = self.conv(x)
        x = x + noise
        mean = x.mean(dim = [2,3], keepdim = True) #NxC
        stddev = x.std(dim = [2,3], keepdim = True)

        x = (x - mean) / (stddev + 1e-8)
        x = x * sy.reshape(N, -1, 1, 1) + sx.reshape(N, -1, 1, 1)
        x = self.act(x)
        return x
...
class StyleLayer(nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, resolution):
        ...
        self.A_block_sy = nn.Linear(w_dim, out_channels)
        self.A_block_sx = nn.Linear(w_dim, out_channels)
    ...
    def forward(self, x, w, noise):
        sy = self.A_block_sy(w)
        sx = self.A_block_sx(w)
        b = self.B_block(noise)
        x = self.conv(x, sy, sx, b)
        return x
...
class StyleBlock(nn.Module):
    def __init__(self,in_channels, out_channels, w_dim, resolution, final_resolution):
        ...
        self.style_layer0 = None if resolution == 4 else StyleLayer(in_channels, out_channels, w_dim, resolution)
...
class Synthesis(nn.Module):
    def __init__(self, latent_dims, final_resolution):
        ...
        self.constant_layer = torch.ones(NUM_FEATURE_MAP[4], 4, 4)
